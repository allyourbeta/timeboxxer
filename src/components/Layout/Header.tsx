  ...
  return (
    <header className="h-14 px-4 bg-theme-secondary border-b border-theme flex items-center">
      ...
      <div className="flex items-center gap-4">
        ...
        {completedToday > 0 && (
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-accent-success/10">
            ...
            <span className="text-sm font-medium text-accent-success">{completedToday}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Utilities */}
        {currentView === 'main' && (
          <button onClick={onCollapseAll} className="btn-icon h-8 w-8" title="Collapse all">
            <ChevronsDownUp className="h-4 w-4" />
          </button>
        )}
        <button onClick={handleThemeToggle} className="btn-icon h-8 w-8" title="Toggle theme">
          {theme === 'dark' ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
        </button>
        <button onClick={handleSignOut} className="btn-icon h-8 w-8" title="Sign out">
